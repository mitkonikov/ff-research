...
net = FFNet([layer1, layer2], device, maximize=not args.minimize)
...
if args.sparsity:
    ...
    def pre_batch(net: FFNet, e: int, b: int):
        ...
        def process(w: torch.Tensor):
            tt = w.detach().flatten()
            q25 = torch.quantile(tt, 0.25)
            q75 = torch.quantile(tt, 0.75)
        ...
        result['HISTOGRAM'] = {
            'layer_0': {
                'weight': process(net.layers[0].weight),
                'bias': process(net.layers[0].bias),
            },
            'layer_1': {
        ...
        result['HISTOGRAM2'] = {
            'layer_0': {
                'weight': process2(net.layers[0].weight),
                'bias': process2(net.layers[0].bias),
            },
            'layer_1': {
