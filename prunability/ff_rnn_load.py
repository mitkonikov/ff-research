...
if args.save_activations_hsv:
    ...
    activations = [
        [torch.zeros(net.layers[1].get_dimensions()).to(device) for layer in range(2)]
        for _ in range(10)
    ]
    
    def save_activation(x: torch.Tensor, layer_idx: int, frame: int):
        if layer_idx == 0 or layer_idx == len(net.layers) - 1:
            return
        # x: [batch_size, num_neurons]
        for i in range(x.size(0)):
            label = current_labels[i].item()
            activations[label][layer_idx - 1] += x[i].detach().abs()
