...
class BPDenseNet(IFF):
    ...
    def sparsity(self, sparsityType: SparsityType):
        result = {
            ...
            'layer_3': {
                'weight': ComputeSparsity(self.layers[7].weight.detach(), sparsityType).item(),
                'bias': ComputeSparsity(self.layers[7].bias.detach(), sparsityType).item()
            },
        }
...
if args.sparsity:
    ...
    def pre_batch(net: BPDenseNet, e: int, b: int):
        ...
        result['HISTOGRAM'] = {
            ...
            'layer_1': {
                'weight': net.layers[3].weight.detach().flatten().cpu().histogram(200).hist.tolist(),
                'bias': net.layers[3].bias.detach().flatten().cpu().histogram(200).hist.tolist(),
            },
            'layer_2': {
                'weight': net.layers[5].weight.detach().flatten().cpu().histogram(200).hist.tolist(),
                'bias': net.layers[5].bias.detach().flatten().cpu().histogram(200).hist.tolist(),
            },
            'layer_3': {
                'weight': net.layers[7].weight.detach().flatten().cpu().histogram(200).hist.tolist(),
                'bias': net.layers[7].bias.detach().flatten().cpu().histogram(200).hist.tolist(),
            },
        }
